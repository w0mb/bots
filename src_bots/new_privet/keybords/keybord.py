...
class Keyboard:
    def __init__(self):
        self.buttons = [[InlineKeyboardButton(text="Подтвердить", callback_data="confirm")]]
    ...
    def delete_link(self, text: str, invite_link: str):
        if isinstance(invite_link, str) and invite_link.startswith("http"):
            self.buttons.remove([InlineKeyboardButton(text=text, url=invite_link)])
