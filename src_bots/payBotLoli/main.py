...
action1_button = InlineKeyboardButton(text="Приват(месяц)", callback_data="action1")
action2_button = InlineKeyboardButton(text="Приват(НАВСЕГДА)💥", callback_data="action2")
action3_button = InlineKeyboardButton(text="Приват(неделя)", callback_data="action3")

action_keyboard = InlineKeyboardMarkup(
    inline_keyboard=[[action1_button], [action2_button], [action3_button]]  # Инлайн-кнопки действий
)
...
async def start_handler(message: Message):
    await message.answer("💜 Лоли приват❤️\n\t\t👻Приветствует вас👻")
...
async def payment_done_handler(query: types.CallbackQuery):
    ...
    try:
        await bot.send_message(chat_id=-1002369604122, text=message_to_send, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[approve_button]]))
...
async def approve_access_handler(query: types.CallbackQuery):
    ...
    access_link = "https://t.me/+0x70TI7F7TY3NThi"
    try:
...
async def action2_handler(query: types.CallbackQuery):
    await query.answer()
    new_text = (
        "Тариф: приват (НАВСЕГДА)\n"
        "Стоимость: 1000.00 🇷🇺RUB\n"
        "Срок действия: 1 месяц\n\n"
        "Вы получите доступ к следующим ресурсам:\n"
        "• Лоли Приват(канал)\n\n"
        "👇ПРИ ПОКУПКЕ ВЫ ПОЛУЧАЕТЕ👇\n\n"
...
@router.callback_query(lambda query: query.data == "action3")
async def action3_handler(query: types.CallbackQuery):
    await query.answer()
    new_text = (
        "Тариф: приват (неделя)\n"
    ...
    # Добавляем кнопки "Оплатить" и "👈назад"
    pay_button = InlineKeyboardButton(text="💳 Оплатить", callback_data="pay_action3")
    back_button = InlineKeyboardButton(text="👈назад", callback_data="back_to_actions")
    back_keyboard = InlineKeyboardMarkup(
        inline_keyboard=[[pay_button], [back_button]]
    )
...
@router.callback_query(lambda query: query.data == "pay_action3")
async def go_to_payment3_handler(query: types.CallbackQuery):
    await query.answer()
    card_number = "💳 Ваш номер карты для оплаты: 2202205347906148\n\nКовалев Д.А.(сбер)\n\n400.00 рублей\n\nПожалуйста, нажмите 'Оплатил', когда завершите оплату и ожидайте доступ."

    # Создаем клавиатуру с кнопкой "Оплатил" и "Отмена"
    paid_button = InlineKeyboardButton(text="💸 Оплатил", callback_data="payment_done3")
    cancel_button = InlineKeyboardButton(text="🚫 Отмена", callback_data="back_to_actions")
    payment_keyboard = InlineKeyboardMarkup(inline_keyboard=[[paid_button], [cancel_button]])
...
# Обработчик для кнопки "Оплатил" в "Действие 2"
@router.callback_query(lambda query: query.data == "payment_done3")
async def payment_done3_handler(query: types.CallbackQuery):
    await query.answer()
    ...
    if file:
        ...
        try:
            if message.photo:
                ...
                await bot.send_photo(chat_id='-1002369604122', photo=file.file_id, caption=caption)
            elif message.document:
                # Отправляем документ с подписью
                await bot.send_document(chat_id='-1002369604122', document=file.file_id, caption=caption)
        finally:
